...
**Next steps (resume here):** M0/M1 (schema, `devices`/`telemetry`/`ingest` functions, Next.js dashboard, simulator container) is implemented and **verified** — schema pushed cleanly (all 9 indexes created), `backend/_generated/` generated, and the dashboard rendered live at `localhost:3000` via `npm run dev` (root, native) and via `docker compose up frontend` (Docker). M2 (auth & roles) is now implemented and test-verified (see Phase M2 below and `specs/auth-roles/spec.md`) — 21/21 backend tests pass, `tsc --noEmit` is clean for `backend/`, `tests/`, and `frontend/`, but it has **not** been pushed to or exercised against a real running Convex deployment (no network path to either Convex Cloud login or a self-hosted backend was available in the implementing sandbox; see the Phase M2 exit-criteria note). Next up: run `npx convex dev` for real against a live deployment to confirm the schema/auth config pushes cleanly and do a manual sign-in pass, then start M3 (alerting).
...
- [x] Wire the chosen auth provider into the frontend and Convex. **Resolved:** Convex Auth (`@convex-dev/auth`) with the Password provider — see `specs/auth-roles/spec.md` §2 for the decision and rationale. Backend: `backend/auth.config.ts`, `backend/auth.ts`, `backend/http.ts`. Frontend: `frontend/app/providers.tsx` (`ConvexAuthProvider`), `frontend/app/page.tsx` (sign-in/sign-up screen via `useAuthActions`).
- [x] Implement `users` table population on first login, `users.me`, `users.list`, `users.setRole`. `backend/users.ts`'s `syncUserOnLogin` (invoked from `auth.ts`'s `createOrUpdateUser` callback) implements the bootstrap rule from spec §5: the first user ever created becomes `admin`, every subsequent first login defaults to `viewer`.
- [x] Add a shared server-side role-check helper used by every protected query/mutation, per spec §11's "server-side role checks on every function; deny-by-default" practice. `backend/lib/auth.ts`: `getCurrentUser`, `requireAuthenticatedUser`, `requireRole` — all denial paths throw the same generic message (non-leakage, spec §6.5 req. 20).
- [x] Retrofit role checks onto every mutation/query from M0–M1 that spec §6.5 requires (`devices.register`/`update`/`deactivate` → admin only; reads scoped by role where applicable). Done in `backend/devices.ts` and `backend/telemetry.ts`; the `TODO(M2)` marker is removed.
- [x] Add role-appropriate UI gating in the frontend (hide/disable actions a role can't perform), while keeping the server check as the actual enforcement point. `frontend/app/page.tsx` gates the register-device form and the deactivate button on `users.me().role === "admin"`.
- [x] Manual security pass: attempt each mutation as each role, confirm unauthorized attempts fail closed without leaking data existence (spec §6.5 req. 20, and success metric in spec §15). Automated in `tests/devices.test.ts` and `tests/users.test.ts` (one test per Role Matrix cell, plus the R4 error-message-parity check) — see `specs/auth-roles/spec.md` §7/§8 for the full test plan and acceptance criteria. 21/21 tests pass (`npm test`).

**Exit criteria:** All functions require authentication; a scripted or manual test matrix (role × function) shows correct allow/deny behavior for all four roles. **Met** — see `tests/devices.test.ts`/`tests/users.test.ts`. One caveat: `npx convex dev`/`codegen` could not be run against a live deployment in the implementing sandbox (no interactive browser login for Convex Cloud; the self-hosted Docker backend's image pull to `ghcr.io` and the CLI's anonymous-local-backend download from `version.convex.dev` were both blocked by the sandbox's egress policy). `backend/_generated/` was instead produced with the installed `convex` package's own offline codegen path (`npx convex codegen --system-udfs --init`, which needs no network or credentials) so the code could be typechecked and tested locally; it must be regenerated for real via `npx convex dev` on first real dev/deploy (it's gitignored, as before).
