...
describe("DeviceCard", () => {
  test("a live device renders no Stale text, and status/last-seen match the input verbatim (R1)", () => {
    ...
    render(<DeviceCard device={baseDevice} now={now} />);

    expect(screen.queryByText(/stale/i)).not.toBeInTheDocument();
    expect(screen.getByText(/status:/i).textContent).toMatch(/online/i);
  });

  test("a device past its staleness threshold renders a text Stale badge (R6)", () => {
  ...
  test('a device that has never reported shows "never" last-seen and is flagged not-live', () => {
    const neverSeen: DeviceCardDevice = { ...baseDevice, lastSeenAt: undefined };
    render(<DeviceCard device={neverSeen} now={2_000_000} />);
